...
extern int redisCommand_HSET_STR(const char* key, const char* field, char* value, int* result);
extern int redisCommand_LRANGE(const char* key, long long start, long long stop, int* result);
